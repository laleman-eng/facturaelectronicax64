IF EXISTS(SELECT name FROM sysobjects                
      WHERE name = 'VID_VW_FE_OINV_D' AND type = 'V')
   DROP VIEW VID_VW_FE_OINV_D
GO--                                                 

CREATE VIEW [dbo].[VID_VW_FE_OINV_D]
AS
	SELECT
		 ...
		 ,CASE WHEN T0.DocSubType IN ('IB', 'EB') THEN CASE
                                                         ...
                                                         ELSE 0
                                                       END
               ELSE CASE WHEN T1.VatSum = 0.0 THEN 1
                         ELSE 2
                    END
		...
		,CASE WHEN T0.DocSubType = 'IX'
			  ...
			  WHEN T0.DocSubType IN ('IB', 'EB') THEN CASE ISNULL(T1.Currency, 'CLP') WHEN 'CLP' THEN T1.PriceAfVAT
		...
		,CASE 
			WHEN ISNULL(T1.DiscPrcnt, 0.0) <= 0.0 THEN 0.0
				   ELSE CASE WHEN T0.DocSubType = 'IX' OR RIGHT(N0.BeginStr, 3) = '110'
	...
	SELECT
		...
		,ISNULL(F0.U_CodImpto, '')																															[CodImpAdic]
		,LEFT(ISNULL(U0.U_NAME, ''), 30)																													[Usuario]
		,ISNULL(T4.TaxSum, 0)																																[MontoImptoAdic]
		,ISNULL(F0.U_Porc, 0.0)																																[PorcImptoAdic]
		,T1.Rate																																			[Rate]
		,T1.Currency																																		[Currency]
		,T1.VisOrder																																		[LineaOrden]
